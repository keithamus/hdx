...
const SNAPSHOTS_PATH: &str = "tasks/coverage/snapshots/postcss_tokens/";

pub struct PostCSSTokenizerTestSuite;

impl LexerSuite<PostCSSTokenizerTestCase> for PostCSSTokenizerTestSuite {
	fn new(_: &AppArgs) -> PostCSSTokenizerTestSuite {
		PostCSSTokenizerTestSuite {}
	}

	fn get_tests(&mut self, _: &AppArgs) -> Vec<PostCSSTokenizerTestCase> {
		let mut files = vec![];
		for path in glob(FIXTURES_GLOB).unwrap().flatten() {
			files.push(PostCSSTokenizerTestCase::new(path));
		}
		files
	}
}

pub struct PostCSSTokenizerTestCase {
	name: String,
	source_path: PathBuf,
	json_path: PathBuf,
	source_text: String,
...
impl LexerCase for PostCSSTokenizerTestCase {
	type Token = Value;

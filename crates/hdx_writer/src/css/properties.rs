...
macro_rules! write_properties {
    {$( $prop: ident, )+} => {
        $(
            impl<'a> WriteCss<'a> for $prop<'a> {
                fn write_css<W: CssWriter>(&self, sink: &mut W) -> Result {
                    sink.write_str(Self::name_as_atom().as_ref())?;
                    sink.write_char(':')?;
        ...
        impl<'a> WriteCss<'a> for Property<'a> {
            fn write_css<W: CssWriter>(&self, sink: &mut W) -> Result {
                match self {
                    $(Property::$prop(value) => value.write_css(sink),)+
                    Property::Custom(value) => value.write_css(sink),
                    Property::Unknown(value) => value.write_css(sink),
                }
            }
        }

...
#[test]
fn value_with_punctuated_optional_group() {
	let syntax = to_valuedef!( <color> [/ <color>]?);
	let data = to_deriveinput! { struct Foo; };
	assert_snapshot!(syntax, data, "value_with_punctuated_optional_group");
}

// #[test]
// fn initial_referring_to_type() {
// 	let syntax = to_valuedef!( <foo> );
// 	let data = to_deriveinput! { struct Foo; };
// 	assert_snapshot!(syntax, data, "value_with_punctuated_optional_group");
// }

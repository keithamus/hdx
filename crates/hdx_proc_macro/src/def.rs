			...
			Self::Combinator(opts, DefCombinatorStyle::Alternatives) => {
				...
				let q = quote! {
					...
					#(#other_if)*
					  ...
					  Err(::hdx_parser::diagnostics::Unexpected(parser.cur(), parser.cur().span()))?
					}
				};
				q
			}
			...
			Self::Multiplier(def, DefMultiplierStyle::Range(range)) => {
				...
				let min_check = match range {
					...
					DefRange::Range(Range { start, .. }) => {
						...
						quote! {
							if i < #n {
								return Err(::hdx_parser::diagnostics::Unexpected(parser.cur(), parser.cur().span()))?
							}
						}
					}
				};
			...
			DefRange::RangeTo(RangeTo { end }) => {
				quote! {
					if #end < #capture_name.into() {
						return Err(::hdx_parser::diagnostics::NumberTooLarge(#end, parser.cur().span()))?
					}
				}
			}
			DefRange::Range(Range { start, end }) => {
				quote! {
					if !(#start..#end).contains(#capture_name) {
						return Err(::hdx_parser::diagnostics::NumberOutOfBounds(#capture_name, "#start..#end", parser.cur().span()))?
					}
				}
			}
			DefRange::RangeFrom(RangeFrom { start }) => {
				quote! {
					if #start > #capture_name.into() {
						return Err(::hdx_parser::diagnostics::NumberTooSmall(#start, parser.cur().span()))?
					}
				}
			}
			DefRange::None => quote! {},

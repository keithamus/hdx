...
impl<T: Parse> Parse for StrWrapped<T> {
	fn parse(input_raw: ParseStream) -> Result<Self> {
		Ok(Self(parse2::<T>(input_raw.parse::<LitStr>()?.value().replace("'", "\"").parse::<TokenStream>()?.into())?))
	}
}
	...
	fn parse(input: ParseStream) -> Result<Self> {
		...
		let ty = match ident.0 {
			...
			atom => {
				...
				let mut str = pascal(atom.to_string());
...
impl Def {
	...
	pub fn generated_data_type(&self) -> DataType {
		match self {
			Self::Type(_) | Self::Optional(_) | Self::Group(_, _) | Self::Multiplier(_, _) => {
	...
	pub fn generate_peek_trait_implementation(&self, ident: &Ident) -> TokenStream {
		let steps = match self {
			...
			Self::Type(ty) => {
				let steps = ty.peek_steps(Some(format_ident!("val")));
				quote! {
					#steps
					Ok(Self(val))
				}
			...
			Self::Combinator(opts, DefCombinatorStyle::Alternatives) => {
				let (keywords, others): (Vec<&Def>, Vec<&Def>) =
					opts.into_iter().partition(|def| matches!(def, Def::Ident(_)));
				let other_if: Vec<TokenStream> = others
					...
					.map(|(i, def)| {
						let peek = def.peek_steps();
							...
							quote! { else if #peek { #parse; Ok(Self::#var) } }
						}
					})
					.collect();
			...
			DataType::SingleUnnamedStruct => match self {
				...
				Self::Multiplier(x, style) => match x.as_ref() {
					Def::Type(ty) => {
						let modname = ty.to_type_name();
						let n = match style {
							DefMultiplierStyle::ZeroOrMore => 0f32,
							DefMultiplierStyle::OneOrMore => 1f32,
							DefMultiplierStyle::OneOrMoreCommaSeparated(_) => 1f32,
							DefMultiplierStyle::Range(DefRange::None) => 0f32,
							DefMultiplierStyle::Range(DefRange::Range(Range { start, .. }))
							| DefMultiplierStyle::Range(DefRange::RangeFrom(RangeFrom { start, .. })) => *start,
							DefMultiplierStyle::Range(DefRange::RangeTo(RangeTo { end, .. })) => *end,
						} as usize;
	...
	fn write_steps(&self, capture: TokenStream) -> TokenStream {
		match self {
			...
			Self::Multiplier(def, style) => {
				...
				quote! { for #name in &#capture { #do_write } }
			}
			Self::Punct(_) => todo!(),
		}
	}
...
impl GeneratePeekImpl for Def {
	fn peek_steps(&self) -> TokenStream {
		match self {
			...
			Self::Combinator(p, _) => {
				let peeks: Vec<TokenStream> = p.iter().map(|p| p.peek_steps()).collect();
				quote! { #(#peeks)||* }
			}
			...
			Self::Multiplier(def, DefMultiplierStyle::Range(range)) => {
				...
				let min_check = match range {
					...
					DefRange::Range(Range { start, .. }) => {
						...
						quote! {
							if i < #n {
								return Err(::hdx_parser::diagnostics::Unexpected(parser.cur(), parser.span()))?
							}
						}
					}
				};
				...
				quote! {
					let i = 0;
					let #capture_name = ::smallvec::smallvec![];
					loop {
						#max_check
						if #peek_steps {
							#steps
			...
			DefRange::RangeTo(RangeTo { end }) => {
				quote! {
					if #end < #capture_name.into() {
						return Err(::hdx_parser::diagnostics::NumberTooLarge(#end, parser.span()))?
					}
				}
			}
			DefRange::Range(Range { start, end }) => {
				quote! {
					if !(#start..#end).contains(#capture_name) {
						return Err(::hdx_parser::diagnostics::NumberOutOfBounds(#capture_name, "#start..#end", parser.span()))?
					}
				}
			}
			DefRange::RangeFrom(RangeFrom { start }) => {
				quote! {
					if #start > #capture_name.into() {
						return Err(::hdx_parser::diagnostics::NumberTooSmall(#start, parser.span()))?
					}
				}
			}
			DefRange::None => quote! {},
...
impl GenerateParseImpl for DefIdent {
	fn parse_steps(&self, capture: Option<Ident>) -> TokenStream {
		let atom = self.to_atom_macro();
		quote! {
			let #capture = parser.next_kind(Kind::Ident).map_or(false, |t| parser.parse_atom_lower(t) == #atom);
		}
	}
}

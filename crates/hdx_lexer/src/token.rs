...
impl Token {
	...
	pub fn new_dimension(
		...
		let mut type_flags = type_flags;
		dbg!(format!("{:#034b}", (type_flags as u32) << 29));
	...
	pub fn is_int(&self) -> bool {
		dbg!(format!("{:#08b}", self.kind_bits()));
		dbg!(format!("{:#034b}", self.flags.bits));
		dbg!("is_int", self.kind_bits() & 0b11100 == 0b00100, self.third_bit_is_set());

...
macro_rules! todo {
	($parser: ident) => {
		Err($crate::diagnostics::Unimplemented($parser.cur().span()))?
	};
}
...
macro_rules! unexpected {
	($parser: ident, $($token: tt)+) => {
		Err($crate::diagnostics::Unexpected($($token)+, $parser.cur().span()))?
	};
	($parser: ident) => {
		Err($crate::diagnostics::Unexpected($parser.cur(), $parser.cur().span()))?
	};
}
...
macro_rules! unexpected_ident {
	($parser: ident, $($atom: tt)+) => {
		Err($crate::diagnostics::UnexpectedIdent($($atom)+.clone(), $parser.cur().span()))?
	};
}
...
macro_rules! unexpected_dimension {
	($parser: ident, $($atom: tt)+) => {
		Err($crate::diagnostics::UnexpectedDimension($($atom)+.clone(), $parser.cur().span()))?
	};
}
...
macro_rules! unexpected_function {
	($parser: ident, $($atom: tt)+) => {
		Err($crate::diagnostics::UnexpectedFunction($($atom)+.clone(), $parser.cur().span()))?
	};
}
...
macro_rules! expect_keyword {
	($parser: ident, $atom:ident) => {
		...
		if atom != $atom {
			$crate::unexpected_ident!($parser, atom)
		}
	};
	($parser: ident, $kw:literal) => {
		let token = *$parser.parse::<$crate::Token![Ident]>()?;
		let atom = $parser.parse_atom_lower(token);
		if atom != hdx_atom::atom!($kw) {
			$crate::unexpected_ident!($parser, atom)
		}
	};
}

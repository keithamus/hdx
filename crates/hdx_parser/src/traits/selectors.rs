...
use crate::{diagnostics, discard, parser::Parser, unexpected, Result, Token, Vec};
...
pub trait SelectorList<'a>: Sized + Parse<'a> {
	...
	fn parse_compound_selector(parser: &mut Parser<'a>) -> Result<Spanned<Vec<'a, Self::SelectorComponent>>> {
		...
		loop {
			...
			if parser.at_end() || matches!(token.kind(), Kind::LeftCurly | Kind::RightParen | Kind::Comma) {
				break;
			}
			...
			if token.kind() == Kind::Whitespace {
				...
				if matches!(next_token_kind, Kind::LeftCurly | Kind::RightParen | Kind::Comma) {
					break;
				}
		...
		return Ok(Spanned { node: selector, span: span.end(parser.pos()) });
	}
...
pub trait SelectorComponent<'a>: Sized {
	...
	fn parse_selector_component(parser: &mut Parser<'a>) -> Result<Self> {
		let peek = parser.peek_with::<Token![Any]>(Include::Whitespace).unwrap();
		match peek.kind() {
			Kind::Ident => match parser.peek_n_with(2, Include::Whitespace) {
				...
				_ => {
					...
					Self::type_from_atom(&atom)
						.ok_or_else(|| diagnostics::UnexpectedTag(atom, parser.cur().span()).into())
				}
			},
			Kind::Hash if peek.hash_is_id_like() => {
				...
				let atom = parser.parse_atom(peek);
				Self::type_from_atom(&atom)
					.ok_or_else(|| diagnostics::UnexpectedId(atom.clone(), parser.cur().span()).into())
			}
					...
					match parser.next_with(Include::Whitespace) {
						t if t.kind() == Kind::Ident => {
							...
							Self::class_from_atom(&atom)
								.ok_or_else(|| diagnostics::UnexpectedIdent(atom, parser.cur().span()).into())
						}
				...
				match peek.kind() {
						...
						match next.kind() {
							Kind::Ident => {
								...
								Self::pseudo_element_from_atom(&atom).ok_or_else(|| {
									diagnostics::UnexpectedPseudoElement(atom.clone(), parser.cur().span()).into()
								})
							}
					...
					Kind::Ident => {
						...
						Self::legacy_pseudo_element_from_token(&atom)
							...
							.ok_or_else(|| diagnostics::UnexpectedPseudoClass(atom.clone(), parser.cur().span()).into())
					}

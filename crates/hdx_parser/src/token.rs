...
macro_rules! kind {
	($iden:ident) => {
		...
		impl<'a> Peek<'a> for $iden {
			fn peek(parser: &mut crate::Parser<'a>) -> Option<Token> {
				parser.peek_kind(Kind::$iden)
			}
		}

		impl<'a> Parse<'a> for $iden {
			fn parse(parser: &mut crate::Parser<'a>) -> Result<Self> {
				parser.next_kind(Kind::$iden).map(|t| t.into())
			}
		}
	};
}
				...
				Ok(parser
					...
					.and_then(|t| {
						if t.char().unwrap() == $ch {
							...
							Err($crate::diagnostics::ExpectedDelim(t, parser.span()))?
						}
					})?
					.into())
			}
...
#[macro_export]
macro_rules! Token {
	[Kind::$ident:ident] => { $crate::token::$iden }
}

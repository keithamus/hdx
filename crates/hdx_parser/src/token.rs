use hdx_lexer::{Kind, QuoteStyle, Span, Token};
...
macro_rules! delegate_to_token {
	($ident:ident) => {
		impl $ident {
			...
			#[inline]
			pub fn to_pairwise(&self) -> Option<hdx_lexer::PairWise> {
				self.0.to_pairwise()
			}
		}
...
macro_rules! make_delim {
	($iden:ident, $ch:literal) => {
		...
		impl<'a> Peek<'a> for $iden {
			fn peek(parser: &crate::Parser<'a>) -> Option<Token> {
				let token = parser.peek_next();
				if token.kind() == Kind::Delim && token.char().unwrap() == $ch {
					Some(token)
				} else {
					None
				}

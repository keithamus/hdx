...
// Component values are more than a single value but we can abuse Is to get the cursor
impl<'a> Is<'a> for ComponentValue<'a> {
...
impl<'a> Parse<'a> for ComponentValue<'a> {
	fn parse(p: &mut Parser<'a>) -> ParserResult<Self> {
		if p.peek::<T![' ']>() {
			...
			p.parse::<Function>().map(Self::Function)

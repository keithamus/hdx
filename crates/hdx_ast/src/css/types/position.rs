	...
	fn build(p: &Parser<'a>, c: Cursor) -> Self {
		if <T![Ident]>::is(p, c) {
			match p.parse_atom_lower(c) {
				atom!("center") => Self::Center(kw::Center::build(p, c)),
				atom!("left") => Self::Left(kw::Left::build(p, c)),
				atom!("right") => Self::Right(kw::Right::build(p, c)),
				atom!("top") => Self::Top(kw::Top::build(p, c)),
				atom!("bottom") => Self::Bottom(kw::Bottom::build(p, c)),
				_ => unreachable!(),
			}

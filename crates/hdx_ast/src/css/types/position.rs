use hdx_atom::atom;
use hdx_parser::{unexpected, unexpected_ident, Parse, Parser, Peek, Result as ParserResult, Token};
...
impl<'a> Peek<'a> for Position {
	fn peek(parser: &Parser<'a>) -> Option<hdx_lexer::Token> {
		parser
			.peek::<Token![Ident]>()
			.or_else(|| parser.peek::<Token![Dimension]>())
			.or_else(|| parser.peek::<Token![Number]>())
	}
}
	...
	fn parse(parser: &mut Parser<'a>) -> ParserResult<Self> {
		let maybe_horizontal = if let Some(token) = parser.peek::<Token![Ident]>() {
			match parser.parse_atom_lower(token) {
				atom!("top") | atom!("bottom") => None,
				_ => parser.parse::<HorizontalPosition>().ok(),
			}
		} else {
			parser.parse::<HorizontalPosition>().ok()
		};

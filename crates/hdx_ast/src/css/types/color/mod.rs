...
use hdx_parser::{todo, Parse, Parser, Peek, Result as ParserResult, ToCursors, T};
...
impl<'a> Parse<'a> for Color {
	fn parse(p: &mut Parser<'a>) -> ParserResult<Self> {
		...
		} else if p.peek::<kw::Currentcolor>() {
			Ok(Self::Currentcolor(p.parse::<kw::Currentcolor>()?))
		} else if p.peek::<kw::Transparent>() {
			Ok(Self::Transparent(p.parse::<kw::Transparent>()?))
		} else if p.peek::<NamedColor>() {

...
use hdx_parser::{
	diagnostics, discard, unexpected, unexpected_ident, AtRule, Parse, Parser, Result as ParserResult, Spanned, Token,
	Vec,
};
...
impl<'a> Parse<'a> for KeyframeSelector {
	fn parse(parser: &mut Parser<'a>) -> ParserResult<Self> {
		...
		if parser.parse_atom(token) == atom!("%") && (0.0..=100.0).contains(&n) {

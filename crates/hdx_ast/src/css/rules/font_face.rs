...
use hdx_parser::{
	diagnostics, unexpected, unexpected_ident, Declaration, Parse, Parser, Result as ParserResult, RuleList, Spanned,
	Token, Vec,
};
...
impl<'a> Parse<'a> for FontFace<'a> {
	fn parse(parser: &mut Parser<'a>) -> ParserResult<Self> {
		...
		if atom != atom!("font-face") {
			Err(diagnostics::UnexpectedAtRule(atom, parser.cur().span()))?
		}
